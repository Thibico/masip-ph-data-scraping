...
class GSheetConnector:
    ...
    def connect_gsheet(self):
        try:
            ...
            raise AttributeError(
                f"Recheck sheet {self.spreadsheet_title} is existing."
            ) from e
    ...
    def get_data_as_df(self, ws: gspread.worksheet.Worksheet | str):
        ...
        try:
            ...
            print(
                f"Shape for {self._current_tab} under {self.spreadsheet_title} : {df.shape}"
            )  ## Should I add logging?
            if df.shape[0] == 0:
                raise ValueError(
                    f"Return shape under {self.spreadsheet_title} is being zero."
                )
            return df
        except APIError:
            print(
                f"API error happened for {self.spreadsheet_title}:{self._current_tab}. Need to wait 60 sec & refresh"
            )
            time.sleep(60)  ## wait 1 min to pass Google quota limit
            print(
                f"After API delayed {self.spreadsheet_title}:{self._current_tab} - Restart!"
            )
    ...
    def load_data(self, df, sheet_name: str):
        # self.connect_gsheet()
        try:
            test_sheet = self._get_sheet.add_worksheet(
                title=sheet_name, rows="10000", cols="10"
            )
        except Exception:
